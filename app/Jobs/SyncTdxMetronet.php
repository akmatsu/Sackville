...
class SyncTdxMetronet implements ShouldBeUnique, ShouldQueue
{
    use Queueable;

    /**
     * Object/sub-object code for Metronet circuits — generic
     * communication/network services, same code the public wifi circuit and
     * mobile cellular plan syncs already resolve to.
     */
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            Log::error('TDX Metronet sync failed.', ['exception' => $e]);
    ...
    /**
     * Reconciles TDX report 984 rows into `tdx_metronet_circuits`, upserting
     * by `AssetID`. Writes are chunked (and each chunk committed in its own
    ...
    protected function sync(): array
    {
        ...
        $rows = $client->getMetronet($token);

        if (empty($rows)) {
            Log::warning('TDX returned no Metronet rows; skipping surplus marking to avoid flagging every circuit as surplus.');
            ...
            DB::transaction(function () use ($chunk, $resolver, &$seenAssetIds, &$synced, &$failed, &$errors): void {
                foreach ($chunk as $row) {
                    try {
                        ...
                        Log::warning('Skipped a TDX Metronet row during sync.', [
                            'asset_id' => $row['AssetID'] ?? null,
                            'exception' => $e,
                        ]);
                    }
        ...
        TdxMetronetCircuit::whereNotIn('tdx_asset_id', $seenAssetIds)
    ...
    protected function syncRow(array $row, TdxRowResolver $resolver): string
    {
        ...
        $circuit = TdxMetronetCircuit::updateOrCreate(
            ...
            [
                ...
                'circuit_number' => $resolver->stringOrNull($row[2676] ?? null),
