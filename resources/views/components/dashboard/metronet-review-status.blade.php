...
new class extends Component
{
    ...
    public function eligibleCount(): int
    {
        ...
        return TdxMetronetCircuit::query()->visibleTo(Auth::user())->reviewable()->count();
    }
...
<a href="{{ route('metronet.reviews') }}" wire:navigate
    ...
    <div class="flex items-start justify-between gap-2">
        <flux:heading size="sm">{{ __('Metronet Circuit Review') }}</flux:heading>
    ...
    @elseif ($this->eligibleCount === 0)
        <flux:text>{{ __('No Metronet circuits need review this cycle.') }}</flux:text>
    @else
